...
//    					System.out.println(message.getSource().getParentElement().getAttributeValue("id"));
    					collaborationGraph.addEdge(currentNode.getId()+"_"+ nextNode.getId(), String.valueOf(currentNode.getId()), String.valueOf(nextNode.getId()), true).setAttribute("ui.label", message.getSource().getParentElement().getAttributeValue("id")+"->"+message.getTarget().getParentElement().getAttributeValue("id")+":"+message.getName());
    					collaborationGraph.getNode(nextNode.getId()).setAttribute("ui.label", nextNode.getId());
    					DFS(nextNode);
        			}
				}
